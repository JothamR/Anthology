...
/// <summary>The Spring Bones node: secondary motion on a chain.</summary>
public class N_SpringBones_Tests
{
    ...
    [Fact]
    public void SpringBones_HoldTheirSwingThroughAPausedFrame()
    {
        Skeleton skeleton = new(new[] { new StringID("Root"), new StringID("Hair1"), new StringID("Hair2") },
            new[] { Skeleton.InvalidIndex, 0, 1 },
            new[]
            {
                Transform3D.Identity,
                new Transform3D(new Float3(0f, 1f, 0f), Quaternion.Identity, Float3.One),
        ...
        var graph = new AnimationGraph();
        graph.SetRoot(graph.AddSpringBones(graph.AddClip(TestClips.Const(skeleton, 0f)), new[] { new StringID("Hair1"), new StringID("Hair2") }, damping: 2f));
        AnimationGraphInstance instance = graph.CreateInstance(skeleton);

        for (int i = 0; i < 5; i++) instance.Update(1f / 60f, Transform3D.Identity);
        var world = Transform3D.Identity;
        for (int i = 1; i <= 10; i++)
        {
            world = new Transform3D(new Float3(0f, 0f, i * 0.05f), Quaternion.Identity, Float3.One);
        ...
        {
            instance.Pose.CalculateModelSpaceTransforms();
            return instance.Pose.GetModelSpaceTransform(2).position;
        }

        Float3 swinging = Tip();
        instance.Update(0f, world);

        Assert.True(swinging.Z < -0.01f, "the chain never swung");
        Assert.Equal(swinging.Z, Tip().Z, 4);
    }
    ...
    public void TheSimulationDoesNotAllocateOnceItIsRunning()
    {
        ...
        // The lowest of several rounds, so the runtime warming up in the background does not count.
        long lowest = long.MaxValue;
        for (int round = 0; round < 5; round++)
        {
            long before = GC.GetAllocatedBytesForCurrentThread();
            Run(instance, 12, Transform3D.Identity);
