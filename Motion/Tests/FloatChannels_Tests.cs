...
/// <summary>Float channels on skeletons, clips, blends and animators.</summary>
public class FloatChannels_Tests
{
    ...
    // A channel belongs to no bone, so zeroing every bone leaves the channel at its own weight, which starts full.
    [Fact]
    public void ABoneMask_LeavesChannelsAtTheirOwnWeight()
    {
        Skeleton skeleton = WithChannels("Smile");
        var result = new Pose(skeleton);
