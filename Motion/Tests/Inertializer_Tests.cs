...
/// <summary>The inertializer: carrying a jump in pose off smoothly over time.</summary>
public class Inertializer_Tests
{
    ...
    [Fact]
    public void Inertializer_TakesOneJumpAsOneJump()
    {
        Skeleton skeleton = TestSkeletons.MakeChain();
        var inertializer = new Inertializer(skeleton);
        var output = new Pose(skeleton);

        int fired = 0;
        for (int frame = 0; frame < 20; frame++)
        {
            Pose target = TestClips.At(skeleton, frame < 5 ? 0f : 1f);
            if (inertializer.DetectJump(target, 0.1f, 0.1f))
            {
                fired++;
                inertializer.Begin(target, 0.5f);
            }
            inertializer.Apply(target, output, 1f / 60f);
        }
